...
namespace {
  ...
  const int KingAttackWeights[] = { 0, 0, 7, 2, 5, 3 };

  // Bonuses for enemy's safe checks
  const int QueenContactCheck = 90;
  ...
  Score evaluate_king(const Position& pos, const EvalInfo& ei) {
    ...
    if (ei.kingAttackersCount[Them])
    {
        ...
        attackUnits =  std::min(75, ei.kingAttackersCount[Them] * ei.kingAttackersWeight[Them])
                     + 9 * ei.kingAdjacentZoneAttacksCount[Them]
                     + 22 * popcount<Max15>(undefended)
